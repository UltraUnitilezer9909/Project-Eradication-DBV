  ...
  def pbStartScene(helptext, minlength, maxlength, initialText, subject = 0, pokemon = nil)
    ...
    if USEKEYBOARD
      @sprites["entry"] = Window_TextEntry_Keyboard.new(
        initialText, 0, -2, 400 - 112, 96, helptext, true
      )
    ...
    when 4   # Storage box
      ...
      @sprites["subject"].altcharset = "Graphics/Pictures/UI/TextEntryUI/icon_storage"
...
class PokemonEntryScene2
  ...
  class NameEntryCursor
    def initialize(viewport)
      ...
      @cursortype = 0
      @cursor1 = AnimatedBitmap.new("Graphics/Pictures/UI/TextEntryUI/cursor_1")
      @cursor2 = AnimatedBitmap.new("Graphics/Pictures/UI/TextEntryUI/cursor_2")
      @cursor3 = AnimatedBitmap.new("Graphics/Pictures/UI/TextEntryUI/cursor_3")
      @cursorPos = 0
      updateInternal
    end
    ...
    def updateCursorPos
      value = @cursorPos
      case value
      when PokemonEntryScene2::MODE1   # Upper case # 768 / 2 - 504 / 2
        @sprite.x = 768 / 2 - 508 / 2 - 2 - 4
        @sprite.y = 122 - 2
        @cursortype = 1
      when PokemonEntryScene2::MODE2   # Lower case
        @sprite.x = 768 / 2 - 508 / 2 + 4 + 68 - 4
        @sprite.y = 122 - 2
        @cursortype = 1
      when PokemonEntryScene2::MODE3   # Accents
        @sprite.x = 768 / 2 - 508 / 2 + (6*2) + (68 * 2) - 2 - 4
        @sprite.y = 122 - 2
        @cursortype = 1
      when PokemonEntryScene2::MODE4   # Other symbols
        @sprite.x = 768 / 2 - 508 / 2 + (6*3) + (68 * 3) - 2 - 4
        @sprite.y = 122 - 2
        @cursortype = 1
      when PokemonEntryScene2::BACK   # Back
        @sprite.x = 768 / 2 - 508 / 2 + 296 - 2 - 4
        @sprite.y = 122 - 2
        @cursortype = 2
      when PokemonEntryScene2::OK   # OK
        @sprite.x = 768 / 2 - 508 / 2 + 296 - 2 + 108 - 4
        @sprite.y = 122 - 2
        @cursortype = 2
      else
        if value >= 0
          @sprite.x = 112 + (42 * (value % PokemonEntryScene2::ROWS)) - 4
          @sprite.y = 204 + (42 * (value / PokemonEntryScene2::ROWS)) - 6
          @cursortype = 0
        end
      end
    end 
  ...
  def pbStartScene(helptext, minlength, maxlength, initialText, subject = 0, pokemon = nil)
    ...
    @@Characters.length.times do |i|
      #@bitmaps[i] = AnimatedBitmap.new(sprintf("Graphics/Pictures/UI/TextEntryUI/overlay_tab_#{i}"))
      @bitmaps[i] = AnimatedBitmap.new(sprintf("Graphics/Pictures/UI/TextEntryUI/overlay_tab_#{i == 0 ? 0 : (i == 1 ? 0 : (i == 2 ? 1 : (i == 3 ? 2 : 1)))}"))
      ...
      COLUMNS.times do |y|
        ROWS.times do |x|
          pos = (y * ROWS) + x
          textPos.push([@@Characters[i][0][pos], 21 + (x * 42), 12 + (y * 42), 2,
                       #Color.new(248, 248, 248), Color.new(72, 80, 88)])
                        Color.new(148, 15, 255), Color.new(99, 0, 180), true])
        end
      end
      pbDrawTextPositions(b, textPos)
      @bitmaps[@@Characters.length + i] = b
    end
    underline_bitmap = BitmapWrapper.new(26, 6)
    underline_bitmap.fill_rect(0, 0, 26, 6, Color.new(99, 0, 180))
    underline_bitmap.fill_rect(2, 2, 22, 2, Color.new(148, 15, 255))
    @bitmaps.push(underline_bitmap)
    # Create sprites
    @sprites = {}
    #@sprites["bg"] = IconSprite.new(0, 0, @viewport)
    #@sprites["bg"].setBitmap("Graphics/Pictures/UI/TextEntryUI/bg")
    addBackgroundPlane(@sprites, "bg", $bgPath, @viewport)
    @sprites["namez"] = IconSprite.new(0, 0, @viewport)
    @sprites["namez"].y = 18
    @sprites["namez"].setBitmap("Graphics/Pictures/UI/TextEntryUI/overlay_name")
    @sprites["textz"] = BitmapSprite.new(Graphics.width, Graphics.height, @viewport)
    zzz = @sprites["textz"].bitmap
    pbSetSystemFont(zzz)
    pbSetSmallFont(zzz)
    case subject
    when 1   # Player # Your name
      meta = GameData::PlayerMetadata.get($player.character_ID)
      if meta
        pbDrawTextPositions(@sprites["textz"].bitmap, [[_INTL("What would be your name?"), 150, 24, 0, Color.new(148, 15, 255), Color.new(99, 0, 180), true]])
        filename = pbGetPlayerCharset(meta.walk_charset, nil, true)
        @sprites["subject"] = TrainerWalkingCharSprite.new(filename, @viewport)
        charwidth = @sprites["subject"].bitmap.width
        charheight = @sprites["subject"].bitmap.height
        @sprites["subject"].x = 78 + 26 - (charwidth / 8)
        @sprites["subject"].y = 18 + 14 * 2 + 26 - (charheight / 4)
      end
    when 2   # Pokémon
      if pokemon
        @sprites["subject"] = PokemonIconSprite.new(pokemon, @viewport)
    ...
    @sprites["bottomtab"] = Sprite.new(@viewport)   # Current tab # Graphics.width, Graphics.height
    @sprites["bottomtab"].x = 112
    @sprites["bottomtab"].y = 200
    @sprites["bottomtab"].bitmap = @bitmaps[@@Characters.length]
    @sprites["toptab"] = Sprite.new(@viewport)   # Next tab
    @sprites["toptab"].x = 112 - 700
    @sprites["toptab"].y = 200
    @sprites["toptab"].bitmap = @bitmaps[@@Characters.length + 1]
    @sprites["controls"] = IconSprite.new(0, 0, @viewport)
    @sprites["controls"].x = 768 / 2 - 508 / 2 - 4
    @sprites["controls"].y = 122
    @sprites["controls"].setBitmap(_INTL("Graphics/Pictures/UI/TextEntryUI/overlay_controls"))
    @init = true
  ...
  def pbDoUpdateOverlay2
    ...
    overlay.clear
    modeIcon = [[_INTL("Graphics/Pictures/fake_sprite"), 44 + @mode * 62 + 4 + @mode * 2, 120, @mode * 60, 0, 60, 44]]
  ...
  def pbDoUpdateOverlay
    ...
    textPositions = [
       [@helptext, 160, 18, false, Color.new(148, 15, 255), Color.new(99, 0, 180)]
    ]
    chars = @helper.textChars
    x = 166
    for ch in chars
      textPositions.push([ch, x, 54, false, Color.new(148, 15, 255), Color.new(99, 0, 180)])
      x += 24
    end
  ...
  def pbChangeTab(newtab = @mode + 1)
    ...
    loop do
      if @sprites["bottomtab"].y < 500
        @sprites["bottomtab"].y += deltaY
        @sprites["bottomtab"].y = 500 if @sprites["bottomtab"].y > 500
      end
      if @sprites["toptab"].x < 112
        @sprites["toptab"].x += deltaX
        @sprites["toptab"].x = 112 if @sprites["toptab"].x > 112
      end
      Graphics.update
      Input.update
      pbUpdate
      break if @sprites["toptab"].x >= 112 && @sprites["bottomtab"].y >= 500
    end
