...
###############################
  def initialize(*args)
    ...
    @search_limiter = 1000 * 2
    # Original method
    zer0_pathfinding_init(*args)
################################
  end
end
...
##########################
# x is an array of string names ["NAME1", "NAME2"]
##########################
	if @x.is_a?(Array)
...
###########################
  ...
  def decidepath(x, y, *args)
	r = nil
	r = Pathfind.new(Node.new(x, y), *args)

	if (r.forcing && !r.route.list.empty?)
      r.collisions = 0
      r.character.paths = [] if r.character.paths == nil

################
#By: ORION - steps to walk
################	
	if r.steps > 0
		if r.route.list.size > r.steps
			last = r.route.list.pop()
#################
# Do not pop a negative value
#################
			if(shortest.route.list.size-(r.steps+1) > 0)
				r.route.list.pop(r.route.list.size-(r.steps+1))
			end
			r.route.list.append(last)
		end
	end
################
	    r.character.paths.push(r) 	   
		r.character.force_move_route(r.route) if r.character.paths.size == 1
 	end
  end
  ...
  def findpathevent(x, y, *args)
	...
	for i in 0...xs.size
		r = Pathfind.new(Node.new(xs[i], ys[i]), *args)
		if !shortest
			shortest = r
			gv = i
		else
			if (shortest.route.list.size > r.route.list.size) && r.route.list.size > 0
				shortest = r
				gv = i
			end
		end
	...
	if (shortest.forcing && !shortest.route.list.empty?)
      shortest.collisions = 0
      shortest.character.paths = [] if shortest.character.paths == nil
      shortest.character.paths.push(shortest) 

################
#By: ORION - the last element is to exit, pop and reappend
################	
	if r.steps > 0

		if shortest.route.list.size > r.steps
			last = shortest.route.list.pop()
#################
# Do not pop a negative value
#################
			if(shortest.route.list.size-(r.steps+1) > 0)
				shortest.route.list.pop(shortest.route.list.size-(r.steps+1))
			end
			shortest.route.list.append(last)
		end
	end
################
    shortest.character.force_move_route(shortest.route) if shortest.character.paths.size == 1

##############
#By: ORION - name of event
##############
	$game_variables[76] = x[gv]
	end
  end
end
...
################################
  def initialize(node, char = -1, range = 0, steps = -1, *callbacks)
################################
  ...
  def create_move_route
	...
	if @steps < 0
		if (@forcing && !@route.list.empty?)
		  @collisions = 0
		  @character.paths = [] if @character.paths == nil
