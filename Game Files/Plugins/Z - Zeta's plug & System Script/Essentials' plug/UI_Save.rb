def pbEmergencySave
  oldscene = $scene
  $scene = nil
  pbMessage(_INTL("Zeta is taking too long... The game will restart :)"))
  return if !$player
  if SaveData.exists?
...
class PokemonSave_Scene
  ...
  def pbStartScreen
    ...
    textColor = ["0FA2FF,006CAF", "FF1C0F,AF0900", "F0F0F8,C8C8D0"][$player.gender]
    locationColor = "90F090,209808"    # green 
    ...
    if $player.has_pokedex
      loctext += _INTL("Unidex<r><c3={1}>{2}/{3}</c3>", textColor, $player.pokedex.owned_count, $player.pokedex.seen_count)
    end
