...
class Game_Temp
  ...
  attr_accessor :speechbubble_arrowP
  ...
  def text=(value)
    if value != nil && value != "" && $game_temp.speechbubble_bubble && $game_temp.speechbubble_bubble > 0
      if $game_temp.speechbubble_bubble == 1
        ...
        resizeToFit2(value,500,100)
        @x = $game_map.events[$game_temp.speechbubble_talking].screen_x - ((@width) / 2)
        @y = $game_map.events[$game_temp.speechbubble_talking].screen_y - (42 + @height) - 26
...
def pbCreateMessageWindow(viewport = nil, skin = nil)
  arrow = nil
  if $game_temp.speechbubble_bubble == 0 && $game_map.events[$game_temp.speechbubble_talking] != nil
    $game_temp.speechbubble_outofrange = true
    $game_temp.speechbubble_arrowP = Viewport.new(0, 0, Graphics.width, Graphics.height)
    $game_temp.speechbubble_arrowP.z = 999999
    arrowP = Sprite.new($game_temp.speechbubble_arrowP)
    event = $game_map.events[$game_temp.speechbubble_talking]
    arrowP.x = event.screen_x - (20/2)#+ event.character_name.size / 2 - arrowP.bitmap.width / 2
    arrowP.y = $game_map.events[$game_temp.speechbubble_talking].screen_y + 1
    arrowP.bitmap = RPG::Cache.load_bitmap("Graphics/Windowskins/", "bw speech2 arrow")
    arrowP.zoom_x = 1
    arrowP.zoom_y = 1
  end
  if $game_temp.speechbubble_bubble==2 && $game_map.events[$game_temp.speechbubble_talking] != nil # Message window set to floating bubble.
    if $game_player.direction==8 # Player facing up, message window top.
      ...
      arrow.x = $game_map.events[$game_temp.speechbubble_talking].screen_x - ((@width) / 2)
...
def pbDisposeMessageWindow(msgwindow)
  ...
  msgwindow.dispose
  $game_temp.speechbubble_arrowP.dispose if $game_temp.speechbubble_arrowP
