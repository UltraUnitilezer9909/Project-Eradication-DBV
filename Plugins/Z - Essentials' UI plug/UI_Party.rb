...
class PokemonPartyConfirmCancelSprite < Sprite
  ...
  def initialize(text, x, y, narrowbox = false, viewport = nil)
    ...
    #if narrowbox
     # @bgsprite.addBitmap("desel", "Graphics/Pictures/UI/PartyUI/icon_cancel_narrow")
     # @bgsprite.addBitmap("sel", "Graphics/Pictures/UI/PartyUI/icon_cancel_narrow_sel")
    #else
    @bgsprite.addBitmap("desel", "Graphics/Pictures/UI/PartyUI/icon_cancel")
    @bgsprite.addBitmap("sel", "Graphics/Pictures/UI/PartyUI/icon_cancel_sel")
    #end
...
class PokemonPartyCancelSprite < PokemonPartyConfirmCancelSprite
  def initialize(viewport = nil)
    super(_INTL("CANCEL"), 638, 390, false, viewport)
  end
end
...
class PokemonPartyConfirmSprite < PokemonPartyConfirmCancelSprite
  def initialize(viewport = nil)
    super(_INTL("CONFIRM"), 638, 390, true, viewport)
  end
end
...
class PokemonPartyCancelSprite2 < PokemonPartyConfirmCancelSprite
  def initialize(viewport = nil)
    super(_INTL("CANCEL"), 638, 390, true, viewport)
  end
end
...
class PokemonPartyPanel < Sprite
  ...
  TEXT_BASE_COLOR_1   = Color.new(148, 15, 255)
  TEXT_SHADOW_COLOR_1 = Color.new(99, 0, 180)
  HP_BAR_WIDTH        = 350
  STATUS_ICON_WIDTH   = 46
  ...
  def initialize(pokemon, index, viewport = nil)
    ...
    self.x = 16 + (index % 2) * Graphics.width / 2
    #self.y = (16 * (index % 2)) + (96 * (index / 2))
    self.y = 8 + (16 * (index / 2)) + (74 * (index / 2)) #16 + 46 * index 
    @panelbgsprite = ChangelingSprite.new(0, 0, viewport)
    @panelbgsprite.z = self.z
    # Rectangular panel (using)
    @panelbgsprite.addBitmap("able", "Graphics/Pictures/UI/PartyUI/panel_rect")
    @panelbgsprite.addBitmap("ablesel", "Graphics/Pictures/UI/PartyUI/panel_rect_sel")
    @panelbgsprite.addBitmap("fainted", "Graphics/Pictures/UI/PartyUI/panel_rect_faint")
    @panelbgsprite.addBitmap("faintedsel", "Graphics/Pictures/UI/PartyUI/panel_rect_faint_sel")
    @panelbgsprite.addBitmap("swap", "Graphics/Pictures/UI/PartyUI/panel_rect_swap")
    @panelbgsprite.addBitmap("swapsel", "Graphics/Pictures/UI/PartyUI/panel_rect_swap_sel")
    @panelbgsprite.addBitmap("swapsel2", "Graphics/Pictures/UI/PartyUI/panel_rect_swap_sel")
    #=
    #@helditemsprite.addBitmap("item", "Graphics/Pictures/UI/PartyUI/icon_item")
    ...
    pbSetSystemFont(@overlaysprite.bitmap)
    @hpbar    = AnimatedBitmap.new("Graphics/Pictures/UI/PartyUI/overlay_hp")
  ...
  def refresh_pokemon_icon
    ...
    @pkmnsprite.y        = self.y + 24
    @pkmnsprite.color    = self.color
    @pkmnsprite.selected = self.selected
  end
=begin #where is the icon path?
...
=end
  ...
  def draw_name
    pbDrawTextPositions(@overlaysprite.bitmap,[[@pokemon.name, 94, 10, 0, TEXT_BASE_COLOR_1, TEXT_SHADOW_COLOR_1, true]])
  end
  ...
  def draw_level
    ...
    pbDrawTextPositions(@overlaysprite.bitmap,[["Lv.#{@pokemon.level.to_s}", 346, 40, 1, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR, true]])
    pbSetSystemFont(@overlaysprite.bitmap)
  end
  ...
  def draw_gender
    ...
    base_color   = (@pokemon.male?) ? Color.new(120, 184, 232) : Color.new(248, 168, 184)
    shadow_color = (@pokemon.male?) ? Color.new(0, 112, 248) : Color.new(232, 32, 16)
    #pbDrawTextPositions(@overlaysprite.bitmap,[[gender_text, 224, 22, 0, base_color, shadow_color]])
    pbSetSmallFont(@overlaysprite.bitmap)
    pbDrawTextPositions(@overlaysprite.bitmap,[[gender_text, 68, 12, 0, base_color, shadow_color, true]])
    pbSetSystemFont(@overlaysprite.bitmap)
  end
  ...
  def draw_hp
    ...
    pbDrawTextPositions(@overlaysprite.bitmap, [[hp_text, 8, 54, 0, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR, true]])
    # HP bar
    if @pokemon.able?
      ...
      hprect = Rect.new(0, hpzone * 22, w, 22) #8
      @overlaysprite.bitmap.blt(10, 64, @hpbar.bitmap, hprect)
    end
  end
  ...
  def draw_status
    ...
    @overlaysprite.bitmap.blt(292, 12, @statuses.bitmap, statusrect)
  end

  def draw_shiny_icon
    return if @pokemon.egg? || !@pokemon.shiny?
    pbDrawImagePositions(@overlaysprite.bitmap,[["Graphics/Pictures/shiny", 270, 6, 0, 0, 16, 16]])
  end

  def draw_annotation
    return if !@text || @text.length == 0
    pbDrawTextPositions(@overlaysprite.bitmap,[[@text, 96, 62, 0, TEXT_BASE_COLOR_1, TEXT_SHADOW_COLOR_1, true]])
  end
...
class PokemonParty_Scene
  ...
  def pbStartScene(party, starthelptext, annotations = nil, multiselect = false, can_access_storage = false)
    ...
    addBackgroundPlane(@sprites, "bg", "UI/PartyUI/bg", @viewport)
    #addBackgroundPlane(@sprites, "var", "UI/PartyUI/Layout", @viewport)
    @sprites["messagebox"] = Window_AdvancedTextPokemon.new("")
    @sprites["messagebox"].z              = 10
    ...
    if @can_access_storage
      pbSetSmallFont(@overlaysprite.bitmap)
      pbDrawTextPositions(@overlaysprite.bitmap,[["Recruit list: [X]", 766, 6, 1, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR, true]])
      pbSetSystemFont(@overlaysprite.bitmap) #@overlayspritetext
    end
    # how to make this text small using pbSetSmallFont?
    #@sprites["storagetext"] = Window_UnformattedTextPokemon.new(@can_access_storage ? _INTL("Recruitment list: [X]") : "")
    #@sprites["storagetext"].z           = 10
    #@sprites["storagetext"].x           = Graphics.width - 224
    #@sprites["storagetext"].y           = -18
    #@sprites["storagetext"].viewport    = @viewport
    #@sprites["storagetext"].baseColor   = TEXT_BASE_COLOR
    #@sprites["storagetext"].shadowColor = TEXT_SHADOW_COLOR
    #@sprites["storagetext"].windowskin  = nil
    #@sprites["storagetext"] = @overlayStorageText
    #pbSetSystemFont(@overlayStorageText)
    @sprites["helpwindow"]              = Window_AdvancedTextPokemon.new(starthelptext) #Window_UnformattedTextPokemon.new(starthelptext)
    @sprites["helpwindow"].viewport     = @viewport
    @sprites["helpwindow"].visible      = true
    @sprites["helpwindow"].z            = 1
  ...
  def pbDisplayConfirm(text)
    ret = -1
    #@sprites["messagebox"].x = MESSAGE_BOX_X
    #@sprites["messagebox"].y = MESSAGE_BOX_Y
    @sprites["messagebox"].text    = text
    @sprites["messagebox"].visible = true
    @sprites["helpwindow"].visible = true
    #@sprites["helpwindow"].x = MESSAGE_BOX_X
    #@sprites["helpwindow"].y = MESSAGE_BOX_Y
    using(cmdwindow = Window_CommandPokemon.new([_INTL("Yes"), _INTL("No")])) {
      cmdwindow.visible = false
      pbBottomRight(cmdwindow)
      cmdwindow.y MESSAGE_BOX_Y #-= @sprites["messagebox"].height
  ...
  def pbShowCommands(helptext, commands, index = 0)
    ...
    helpwindow.x       = MESSAGE_BOX_X
    helpwindow.y       = MESSAGE_BOX_Y
  ...
  def pbChangeSelection(key, currentsel)
    ...
    when Input::DOWN
      if currentsel >= Settings::MAX_PARTY_SIZE - 1
        currentsel += -1
        while currentsel < numsprites && !@party[currentsel]
          currentsel += -1
        end
      else
        currentsel += -1
...
class PokemonPartyScreen
  ...
  def pbPokemonGiveScreen(item)
    @scene.pbStartScene(@party, _INTL("Give to which member?"))
  ...
  def pbPokemonGiveMailScreen(mailIndex)
    ...
    if pkmnid >= 0
      ...
      if pkmn.hasItem? || pkmn.mail
        pbDisplay(_INTL("This member has an item. It can't hold mail."))
      elsif pkmn.egg?
  ...
  def pbPokemonMultipleEntryScreenEx(ruleset)
    annot = []
    statuses = []
    ordinals = [_INTL("Unable"), _INTL("Unregistered"), _INTL("Restricted")]
    positions = [_INTL("1st"), _INTL("2nd"), _INTL("3rd"), _INTL("4th"),
                 _INTL("5th"), _INTL("6th"), _INTL("7th"), _INTL("8th"),
                 _INTL("9th"), _INTL("10th"), _INTL("11th"), _INTL("12th")]
    Settings::MAX_PARTY_SIZE.times do |i|
    ...
    @scene.pbStartScene(@party, _INTL("Choose a member and confirm."), annot, true) if $DEBUG
    loop do
      realorder = []
      @party.length.times do |i|
      ...
      @scene.pbSetHelpText(_INTL("Choose a member and confirm.")) if $DEBUG
      pkmnid = @scene.pbChoosePokemon
      addedEntry = false
      ...
      command = @scene.pbShowCommands(_INTL("Do what with {1}?", pkmn.name), commands) if pkmn || $DEBUG
      if cmdEntry >= 0 && command == cmdEntry
        if realorder.length >= ruleset.number && ruleset.number > 0
          pbDisplay(_INTL("No more than {1} may enter.", ruleset.number))
        else
        ...
        @scene.pbSummary(pkmnid) {
          @scene.pbSetHelpText((@party.length > 1) ? _INTL("Choose a member.") : _INTL("Choose a member or cancel.")) if $DEBUG
        }
      end
    end
    @scene.pbEndScene
  ...
  def pbChooseAblePokemon(ableProc, allowIneligible = false)
    ...
    @scene.pbStartScene(
      @party,
      (@party.length > 1) ? _INTL("Choose a member.") : _INTL("Choose a member or cancel."),
      annot
    ) if $DEBUG
    loop do
      @scene.pbSetHelpText(
      ...
      if !eligibility[pkmnid] && !allowIneligible
        pbDisplay(_INTL("This member can't be chosen."))
      else
        ret = pkmnid
        break
      end
  ...
  def pbPokemonScreen
    ...
    @scene.pbStartScene(@party,
                        (@party.length > 1) ? _INTL("Choose a member.") : _INTL("Choose a member or cancel."),
                        nil, false, can_access_storage) if $DEBUG
    # Main loop
    loop do
      ...
      choice = @scene.pbShowCommands(_INTL("Do what with {1}?", pkmn.name), command_list) if $DEBUG
      ...
      when Integer   # Hidden move's index
        ...
        if [:MILKDRINK, :SOFTBOILED].include?(move.id)
          ...
          @scene.pbSetHelpText(_INTL("Use on which member?")) if $DEBUG
          ...
          loop do
            ...
            if party_idx == old_party_idx
              ...
              pbDisplay(_INTL("{1} can't be used on that member.", movename))
            else
              pkmn.hp -= amt
          ...
          if pbConfirmUseHiddenMove(pkmn, move.id)
            ...
            if move.id == :FLY
              ...
              @scene.pbStartScene(
                @party, (@party.length > 1) ? _INTL("Choose a member.") : _INTL("Choose a member or cancel.")
              ) if $DEBUG
              next
            end
            return [pkmn, move.id]
...
MenuHandlers.add(:party_menu, :summary, {
  ...
  "effect"    => proc { |screen, party, party_idx|
    screen.scene.pbSummary(party_idx) {
      screen.scene.pbSetHelpText((party.length > 1) ? _INTL("Choose a member.") : _INTL("Choose a member or cancel."))
    }
  }
})
...
MenuHandlers.add(:party_menu, :switch, {
  "name"      => _INTL("Switch"),
  "order"     => 30,
  "condition" => proc { |screen, party, party_idx| next party.length > 1 && party_idx > 0},
  "effect"    => proc { |screen, party, party_idx|
    ...
    if party_idx > 0
      ...
      pbMessage("Can't change yourself from first position!")
    end
  }
})
...
def pbChoosePokemon(variableNumber, nameVarNumber, ableProc = nil, allowIneligible = false)
  ...
  pbFadeOutIn {
    ...
    if ableProc
      ...
      screen.pbStartScene(_INTL("Choose a member."), false)
